<?php
include_once '../dbconnect.php';
$id_konsumen = $_GET['id_konsumen'];


$query = "SELECT * FROM tb_konsumen WHERE id_konsumen='$id_konsumen'";
$execute = mysqli_query($conn, $query);
$check = mysqli_affected_rows($conn);
$response = array();
if ($check>0){
    $data = array();
    while($retrieve= mysqli_fetch_object($execute)) {
        $bind["id_konsumen"]=$retrieve->id_konsumen;
        $bind["nama_konsumen"]=$retrieve->nama_konsumen;
        $bind["alamat_konsumen"]=$retrieve->alamat_konsumen;
        $bind["nohp_konsumen"]=$retrieve->nohp_konsumen;
        $bind["username"]=$retrieve->username;
        $bind["password"]=$retrieve->password;
        array_push($response, $bind);
    }
} else {
    $response["code"]=0;
    $response["message"]="Data makanan tidak ditemukan";
}
echo json_encode($response);
mysqli_close($conn);